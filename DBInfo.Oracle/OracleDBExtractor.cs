...
namespace DBInfo.DBExtractors {
  public class OracleDBExtractor : IDBInfoExtractor {
    ...
    public DataSet PegarDatasetTabelas() {
      return new DataSet();
      //			SqlCommand qry = new SqlCommand("select name, case when ident_seed(name) is null then 0 else 1 end HasIdentity, ident_seed(name) IdentSeed, ident_incr(name) IdentIncr from sysobjects where xtype = 'U' and uid = 1 order by name", SqlConn);
